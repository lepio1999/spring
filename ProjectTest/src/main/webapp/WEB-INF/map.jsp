      ...
      function success(position) {
        ...
        kakao.maps.event.addListener(map, "click", function (mouseEvent) {
          ...
          const currentLocationElement =
            modalContent.querySelector("#current-location");
          ...
          if (locationsWithinRange.length === 1) {
            ...
            let userChoice = prompt(
              `여러 지역이 ${desiredRange}km 이내에 있습니다. 아래에서 선택해주세요:\n${locationsWithinRange
                .map(
                  (loc, index) =>
                    `${index + 1}. ${loc.loc_code} - ${loc.detail_loc}`
                )
                .join("\n")}`
            );
            ...
            if (userChoice && userChoice <= locationsWithinRange.length) {
              return `${locationsWithinRange[userChoice - 1].loc_code} - ${
                locationsWithinRange[userChoice - 1].detail_loc
              }`;
            }
          }
        }
